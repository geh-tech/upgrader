...
// Базовые множители для комбинаций
...
// Улучшения рук (хранятся в прогрессии)
let handUpgrades = {
    high: 0,
    pair: 0,
    twoPair: 0,
...
let passiveBonuses = {
    bones: 0,      // +X к костям каждый раунд
    mult: 0,       // +X к множителю каждый раунд
    rerolls: 0     // +X к перебросам
};

// Список всех возможных улучшений для выбора
const ALL_UPGRADES = [
    // Улучшения рук
...
function startRound() {
    roundActive = true;
    hasRolled = true;
    // Применяем пассивные бонусы к перебросам
...
function playHand() {
    ...
    // Рассчитываем результат с учетом пассивных бонусов
    ...
    if (isWin) {
        ...
        // Повышаем уровень
        level++;
        // Лимит растет быстрее, чтобы соответствовать прокачке
        ...
        // === НОВОЕ: ПОКАЗЫВАЕМ ВЫБОР УЛУЧШЕНИЙ ===
        showUpgradeModal();
...
// ===== НОВЫЕ ФУНКЦИИ ДЛЯ ПРОКАЧКИ =====

// Показать модальное окно с выбором улучшений
function showUpgradeModal() {
    ...
    // Сохраняем выбранные улучшения в глобальную переменную для доступа из onclick
...
function applyUpgrade(index) {
    ...
    // Сохраняем прогресс с новыми улучшениями
    saveProgress();
...
// Получить название руки по типу
